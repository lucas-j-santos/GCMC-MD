...
for k in range(points):
    ...
    for i in range(iterations-1):
        ...
        df = pd.read_csv(f'pressure_{pressure[k]:.2f}_Pa/npt_output_{i}.csv')
        Lz_it[i] = df['Box-Z'].mean()

    for i in range(iterations):

        with open(f'pressure_{pressure[k]:.2f}_Pa/gcmc_output_{i}.data', 'r') as file:
            content = file.read()
...
df = pd.DataFrame({
    ...
    'Unit cell length (Angstrom)': Lz/2,
...
df.to_csv(f'Ar_{framework_name}_{T:.2f}K_GCMC-MD.csv')
