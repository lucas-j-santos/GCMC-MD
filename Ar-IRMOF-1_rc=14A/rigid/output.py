...
Nads = np.empty_like(pressure)

for i in range(len(pressure)):

    with open(f'pressure_{pressure[i]:.2f}_Pa/output.data', 'r') as file:
        content = file.read()
...
with plt.style.context('seaborn-v0_8'):
    plt.plot(1e-3*pressure, Nads*(1e3/framework_mass),'C0o', markersize=7, markeredgewidth=1.5, mfc='none')
    plt.xlabel('Pressure (kPa)')
    plt.ylabel('Absolute adsorption (mol/kg)')
plt.show()

df = pd.DataFrame()
df['Pressure (Pa)'] = pressure
df['Absolute adsorption (molecules/uc)'] = Nads
df['Absolute adsorption (mol/kg)'] = Nads*(1e3/framework_mass)
df.to_csv(f'argon_{framework_name}_{T:.2f}K.csv')