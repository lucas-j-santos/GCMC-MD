...
def write_inp(framework_name,temperature,pressure,rc,molecule_name,fugacity_coefficient,create_molecules):
    ...
    with open('simulation.input', 'w+') as f:
        ...
        f.write(f"""NumberOfInitializationCycles 5000      
...
temperature = 78.0
# pressure = torch.tensor([10.0, 100.0, 200.0, 400.0, 600., 800., 1000.0, 2000.0, 4000.0, 6000.0, 8000.0, 
#                10000.0, 12000.0, 14000.0, 16000.0, 18000.0, 20000.0, 24000.0, 28000.0, 30000.0, 32000.0])
#pressure = torch.hstack((torch.tensor([1.0, 5.0]), torch.arange(1e1, 2e2, 1e1), torch.arange(2e2, 1e3, 1e2),
#                         torch.arange(1e3, 1.1e4, 1e3)))  

pressure = torch.tensor([66.0])
...
# df.to_csv(f'{molecule_name}_{framework_name}_{temperature:.2f}K.csv')
